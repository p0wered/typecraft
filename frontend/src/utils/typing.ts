...
export type KeyMistakes = Record<string, number>;

export function normalizeKeyLabel(char: string): string {
...
export function recordKeyMistake(
  mistakes: KeyMistakes,
  expectedChar: string | undefined,
  fallbackChar: string,
) {
  const label = normalizeKeyLabel(expectedChar ?? fallbackChar);
  mistakes[label] = (mistakes[label] ?? 0) + 1;
}
...
export function calculateConsistency(wpmSamples: number[]): number {
  ...
  if (mean === 0) return 0;
  const variance =
    wpmSamples.reduce((acc, v) => acc + (v - mean) ** 2, 0) / wpmSamples.length;
