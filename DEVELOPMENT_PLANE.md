...
- [x] Инициализация монорепо (npm workspaces)
- [x] Настройка Vite + React + TypeScript (frontend)
- [x] Настройка Express + TypeScript (backend)
- [x] Общие типы (shared/)
- [x] Настройка ESLint, Prettier, Vitest
- [x] Базовый layout: Header + роутинг страниц

### Фаза 2: Ядро — механика печати

- [x] Компонент TypingArea (посимвольный ввод, подсветка)
- [x] Хук useTyping (логика ввода, сравнение, статистика в реальном времени)
- [x] Режим Words (генерация слов, подсчёт WPM/accuracy)
- [x] Режим Time (таймер обратного отсчёта)
- [x] Экран результатов с базовой статистикой
- [x] Хранение настроек и результатов в Zustand + localStorage (пока без бэка)

### Фаза 3: Бэкенд и аутентификация

- [x] Схема БД (Drizzle + SQLite)
- [x] Auth-эндпоинты (register, login, JWT)
- [x] CRUD для результатов
- [x] API для настроек
- [x] Интеграция фронта с API

### Фаза 4: Режимы Quote и Code

- [x] Коллекция цитат (JSON)
- [x] Коллекция код-сниппетов по языкам (JSON)
- [x] Адаптация TypingArea для многострочного кода
- [x] UI выбора языка программирования
